...
_SECTION_MAP: List[Tuple[str, str, str, List[str]]] = [
    ...
    ("general_description.intended_purpose", "1c", "General description — intended purpose",
     ["Article 6(2)", "Article 13(3)(b)", "Annex IV §1(c)", "Article 43(4)"]),
    ("general_description.provider", "1a", "General description — provider",
     ["Article 13(3)(a)", "Annex IV §1(a)"]),
    ("general_description.system", "1b", "General description — system identity",
     ["Article 13(3)(b)", "Annex IV §1(b)", "Article 43(4)"]),
    ("general_description", "1", "General description",
     ["Article 13(3)", "Annex IV §1"]),
    ("development_process.data_governance", "2b", "Development process — data governance",
     ["Article 10", "Article 10(2)(f)", "Annex IV §2", "Article 43(4)"]),
    ("development_process", "2", "Development process",
     ["Article 10", "Annex IV §2", "Article 43(4)"]),
    ("monitoring_functioning_control.accuracy_metrics", "3a", "Monitoring — accuracy metrics",
     ["Article 15", "Article 15(4)", "Annex IV §3", "Article 43(4)"]),
    ("monitoring_functioning_control.human_oversight_measures", "3b", "Monitoring — human oversight",
    ...
    ("risk_management", "4", "Risk management",
     ["Article 9", "Article 9(2)", "Annex IV §4", "Article 43(4)"]),
    ("lifecycle_changes", "5", "Lifecycle changes",
     ["Article 16(d)", "Annex IV §5"]),
    ("harmonised_standards", "6", "Harmonised standards",
     ["Annex IV §6", "Article 43(1)"]),
    ("eu_declaration_of_conformity", "7", "EU declaration of conformity",
     ["Article 47", "Annex IV §7"]),
    ("post_market_monitoring", "8", "Post-market monitoring",
     ["Article 72", "Article 72(1)", "Annex IV §8"]),
    ("serious_incidents", "9", "Serious incidents",
     ["Article 73", "Annex IV §9"]),
...
_FACTOR_DEFINITIONS = [
    ...
    },
    {
        ...
        "triggers": ["development_process.data_governance.training_sources",
    ...
    },
    {
        ...
        "triggers": ["development_process.architecture_description",
...
def _compute_factors(entry_paths: List[str]) -> List[SubstantialityFactor]:
    ...
    for defn in _FACTOR_DEFINITIONS:
        ...
        factors.append(SubstantialityFactor(
            id=str(defn["id"]),
...
def diff_dossiers(
    ...
    for k in sorted(all_keys):
        ...
        entries.append(DiffEntry(
            ...
            old_value=old_val,
            new_value=new_val,
            citations=arts + ["Article 43(4)"] if "Article 43(4)" not in arts else arts,
            substantiality=cast(
                Literal["substantial", "non_substantial", "ambiguous", "unknown"], subst
            ),
        ))

    has_substantial = any(e.substantiality == "substantial" for e in entries)
    reg_changed = (
        _field_str(old.general_description.system.regulation_version)
