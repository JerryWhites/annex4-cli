...
from annex4.cli.disclaimer import print_cli_disclaimer, FULL_DISCLAIMER, DisclaimerRequiredError
...
@cli.command()
@click.option("--output", "-o", "output_path", type=click.Path(path_type=Path, dir_okay=False, writable=True), help="Path to save the RiskProfile JSON.")
@click.option("--regulation-version", help="Regulation version ID to use.", default="2024-1689_base", show_default=True)
@click.option("--i-acknowledge-uncertainty", "acknowledged", is_flag=True, default=False, help="Required flag: confirms you understand this output is not a legal determination.")
@click.option("--system", "system_yaml", type=click.Path(exists=True, path_type=Path, dir_okay=False), default=None, help="Path to an existing dossier YAML — enables non-interactive classification.")
def classify(output_path: Optional[Path], regulation_version: str, acknowledged: bool, system_yaml: Optional[Path]) -> None:
    ...
    if not acknowledged:
        console.print(Panel(
            ...
            border_style="yellow", title="annex4 classify",
        ))
        _sys.exit(2)
    ...
    if system_yaml:
        console.print(f"[dim]Reading system from {system_yaml} (non-interactive mode)…[/dim]")
        profile = engine.classify_from_yaml(system_yaml)
    else:
        console.print(Panel("[bold]EU AI Act Risk Classifier[/bold]\n\nAnswer the following questions to determine the risk profile of your AI system.\nYour answers are processed locally — nothing is transmitted.", title="Classification", border_style="green"))
        profile = engine.run()
    ...
    nb_text = "[bold red]Yes[/bold red]" if profile.notified_body_required else "[green]No (by default)[/green]"
    ...
    console.print()
    console.print(Panel(f"[dim]{profile.disclaimer}[/dim]", border_style="yellow", title="⚠  Not legal advice"))

    if output_path:
        result_data = {
            "risk_profile": profile.model_dump(),
            "answers": {k: list(v) if isinstance(v, set) else v for k, v in engine.answers.items()},
...
@cli.command()
@click.argument("classification_file", type=click.Path(exists=True, path_type=Path, dir_okay=False))
@click.option("--output", "-o", "output_path", type=click.Path(path_type=Path, dir_okay=False, writable=True), required=True)
@click.option("--template", default="annex_iv.md.j2", show_default=True)
@click.option("--overrides", "override_paths", type=click.Path(exists=True, path_type=Path, dir_okay=False), multiple=True)
def generate(classification_file: Path, output_path: Path, template: str, override_paths: tuple[Path, ...]) -> None:
    ...
    try:
        ...
        output_path.write_text(engine.render(classification_file, template, list(override_paths)), encoding="utf-8")
        console.print(f"[bold green]Success![/bold green] Documentation generated at [cyan]{output_path.resolve()}[/cyan]")
...
def _print_validation_report(console: Console, report: Any) -> None:
    ...
    level_style = {Level.ERROR: "bold red", Level.LEGAL_GAP: "bold magenta", Level.INCONSISTENCY: "bold yellow", Level.WARNING: "yellow", Level.ASSUMPTION: "dim yellow", Level.INFO: "dim cyan"}
    level_label = {Level.ERROR: "ERROR  ", Level.LEGAL_GAP: "LGL_GAP", Level.INCONSISTENCY: "INCONSIST", Level.WARNING: "WARNING", Level.ASSUMPTION: "ASSUMPT", Level.INFO: "INFO   "}
    ...
    for issue in report.issues:
        ...
        console.print(f"[{style}]{level_label[issue.level]}[/{style}]  [{style}]{issue.code}[/{style}]{ref}")
    ...
    console.print("-" * 72)
    console.print(f"[{summary_style}]{e} error(s)[/{summary_style}]  [yellow]{w} warning(s)[/yellow]  [dim cyan]{i} info[/dim cyan]")


@cli.command(name="validate")
@click.argument("dossier_file", type=click.Path(exists=True, path_type=Path, dir_okay=False))
@click.option("--strict", is_flag=True, default=False, help="Exit with code 1 if there are any warnings (not just errors).")
def validate_dossier(dossier_file: Path, strict: bool) -> None:
    ...
    console = Console()
    try:
        dossier = AnnexIVDossier.from_yaml_dict(yaml.safe_load(open(dossier_file, encoding="utf-8")) or {})
    except Exception as e:
...
@cli.command()
@click.argument("dossier_file", type=click.Path(exists=True, path_type=Path, dir_okay=False))
@click.option("--output", "-o", "output_path", type=click.Path(path_type=Path, dir_okay=False, writable=True), required=True, help="Path to save the generated document.")
@click.option("--format", "fmt", type=click.Choice(["markdown", "html", "pdf"], case_sensitive=False), default=None, help="Output format. Inferred from --output extension if not specified.")
@click.option("--no-disclaimer", "no_disclaimer", is_flag=True, default=False, hidden=True, help="[BLOCKED] The disclaimer cannot be disabled.")
def render(dossier_file: Path, output_path: Path, fmt: Optional[str], no_disclaimer: bool) -> None:
    """Generate a complete Annex IV document from a filled dossier YAML file."""
    if no_disclaimer:
        raise DisclaimerRequiredError("The disclaimer cannot be disabled. All annex4-cli outputs must carry the mandatory legal disclaimer.")
    ...
    if fmt is None:
        ...
        fmt = {".html": "html", ".pdf": "pdf", ".md": "markdown"}.get(suffix, "markdown")
    ...
    try:
        ...
        if not report.is_valid:
            ...
            raise click.Abort()
        if report.warnings:
            console.print(f"[yellow]Continuing with {len(report.warnings)} warning(s) — the document will be generated but may be incomplete.[/yellow]\n")
        if fmt == "html":
            ...
            output_path.write_text(GeneratorEngine().render_dossier(dossier_file), encoding="utf-8")
        console.print(f"[bold green]Success![/bold green] Annex IV document ({fmt}) generated at [cyan]{output_path.resolve()}[/cyan]")
    except click.Abort:
        raise
    except Exception as e:
        console.print(f"[bold red]Error:[/bold red] {e}")
...
@click.option("--override", "override_paths", type=click.Path(exists=True, path_type=Path, dir_okay=False), multiple=True)
@click.option("--output", "-o", "output_path", type=click.Path(path_type=Path, dir_okay=False, writable=True), default="dossier.yaml", show_default=True)
def ingest(mlflow_run_id: Optional[str], mlflow_uri: Optional[str], hf_model_id: Optional[str], hf_token: Optional[str], override_paths: tuple[Path, ...], output_path: Path) -> None:
    ...
    if not mlflow_run_id and not hf_model_id and not override_paths:
        console.print("[bold red]Error:[/bold red] Specify at least one source: --mlflow-run, --hf-model, or --override.")
        sys.exit(1)
    ...
    if mlflow_run_id:
        try:
            ...
            outputs.append(MLflowIngestor(tracking_uri=mlflow_uri).ingest(run_id=mlflow_run_id))
    ...
    if hf_model_id:
        try:
            ...
            outputs.append(HuggingFaceIngestor().ingest(model_id=hf_model_id, token=hf_token))
...
@cli.command()
@click.argument("old_file", type=click.Path(exists=True, path_type=Path, dir_okay=False))
@click.argument("new_file", type=click.Path(exists=True, path_type=Path, dir_okay=False))
@click.option("--format", "fmt", type=click.Choice(["cli", "json", "markdown", "html"]), default="cli")
@click.option("--output", "-o", "output_path", type=click.Path(path_type=Path, dir_okay=False, writable=True), default=None, help="Write report to file instead of stdout.")
def diff(old_file: Path, new_file: Path, fmt: str, output_path: Optional[Path]) -> None:
    ...
    reg_version = _field_str(old_dossier.general_description.system.regulation_version) or "2024-1689_base"
    ...
    if fmt == "cli":
        if report.regulation_version_changed:
            console.print(Panel("[bold yellow]Regulation version changed between dossiers![/bold yellow]"))
        if not report.entries:
            ...
            for entry in report.entries:
                sub_style = "red" if entry.substantiality == "substantial" else ("yellow" if entry.substantiality == "ambiguous" else "green")
                table.add_row(entry.path, entry.kind, f"[{sub_style}]{entry.substantiality}[/{sub_style}]", str(entry.new_value) if entry.new_value is not None else "")
            console.print(table)
    elif fmt == "json":
        ...
        try:
            rendered = env.get_template(template_name).render(report=report.model_dump())
            if output_path:
...
def bundle_create(dossier_file: Path, output_path: Path) -> None:
    ...
    try:
        ...
        console.print(f"[bold green]Bundle created:[/bold green] [cyan]{output_path.resolve()}[/cyan]")
...
@click.argument("bundle_file", type=click.Path(exists=True, path_type=Path, dir_okay=False))
def bundle_verify(bundle_file: Path) -> None:
    ...
    if result["ok"]:
        console.print(f"[bold green]✓ Bundle verified:[/bold green] {result['checked']} file(s) intact.")
    else:
...
def list_regulations() -> None:
    ...
    try:
        ...
        versions = [d.name for d in versions_path.iterdir() if d.is_dir() and d.name != "__pycache__"]
...
def show(document_id: str, version: str) -> None:
    ...
    document: Any = None
    if doc_type == "article":
        document = next((d for d in regulation_obj.articles if d.identifier == doc_identifier), None)
    elif doc_type == "annex":
        document = next((d for d in regulation_obj.annexes if d.identifier == doc_identifier), None)
    elif doc_type == "recital":
        document = next((d for d in regulation_obj.recitals if d.identifier == doc_identifier), None)
    if document:
        console.print(Panel(f"[bold]{getattr(document, 'title', 'Recital')}[/bold]\n\n{document.text}", title=document_id, border_style="blue"))
    else:
...
@click.option("--output-dir", "-o", type=click.Path(path_type=Path, file_okay=False, writable=True), default="schemas", show_default=True)
def generate_schemas_command(output_dir: Path) -> None:
    ...
    try:
        ...
        console.print(f"[bold green]Success![/bold green] Schemas generated in [cyan]{output_dir.resolve()}[/cyan]")
    except Exception as e:
        console.print(f"[bold red]Error:[/bold red] {e}")
        raise click.Abort()
...
@cli.command()
@click.option("--output", "-o", "output_path", type=click.Path(path_type=Path, dir_okay=False, writable=True), default="dossier.yaml", show_default=True)
@click.option("--role", type=click.Choice(["provider", "deployer"], case_sensitive=False), default="provider", show_default=True)
def init(output_path: Path, role: str) -> None:
    ...
    try:
        ...
        console.print(f"[bold green]Success![/bold green] New dossier created at [cyan]{output_path.resolve()}[/cyan]")
    except Exception as e:
        console.print(f"[bold red]Error:[/bold red] {e}")
        raise click.Abort()
